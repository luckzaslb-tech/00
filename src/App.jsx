...
function localAI(msg,lancs){
  ...
  // sem valor = conversa
  if(!valor){
    const pergW=["quanto","gastei","recebi","resumo","relatorio","saldo","como estou","quanto tenho","me mostra"];
    if(pergW.some(w=>t.includes(norm(w)))){return{action:"conversa",isSummary:true};}
    return{action:"conversa",resposta:'Não entendi 😕 Tente: "Gastei 50 no mercado" ou "Recebi 3000 de salário"'};
  }
...
const getCSS=(theme)=>`
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  html{height:100%;height:-webkit-fill-available;overflow:hidden;position:fixed;width:100%}
  body{background:${theme==="light"?LIGHT.bg:DARK.bg};color:${theme==="light"?LIGHT.text:DARK.text};font-family:'Figtree',sans-serif;-webkit-tap-highlight-color:transparent;-webkit-font-smoothing:antialiased;height:100%;height:-webkit-fill-available;overflow:hidden;position:fixed;width:100%;overscroll-behavior:none;transition:background .2s,color .2s}
  #root{height:100%;height:-webkit-fill-available;overflow:hidden;position:fixed;width:100%;display:flex;flex-direction:column}
...
function Head({view,onRec,onDep,user,onLogout,theme,onToggleTheme}){
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,zIndex:200,height:HH,background:G.card,borderBottom:`1px solid ${G.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 16px"}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        ...
        <div style={{position:"relative",display:"flex",alignItems:"center",gap:6}}>
          <button onClick={onLogout} className="press" title="Perfil"
          ...
          <button onClick={()=>setMenuOpen(v=>!v)} className="press"
            style={{width:28,height:28,borderRadius:8,border:"none",background:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3.5,padding:0,flexShrink:0}}>
            {[0,1,2].map(i=><div key={i} style={{width:3.5,height:3.5,borderRadius:"50%",background:G.muted}}/>)}
          </button>
          {menuOpen&&<>
            <div onClick={()=>setMenuOpen(false)} style={{position:"fixed",inset:0,zIndex:299}}/>
            <div style={{position:"absolute",top:38,right:0,zIndex:300,background:G.card,border:`1px solid ${G.border2}`,borderRadius:14,padding:6,minWidth:180,boxShadow:"0 8px 32px rgba(0,0,0,.4)",animation:"popIn .15s ease"}}>
              <div onClick={()=>{onToggleTheme();setMenuOpen(false);}} className="press"
                style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:10,cursor:"pointer",fontSize:14,fontWeight:500}}>
                <span style={{fontSize:18}}>{theme==="dark"?"☀️":"🌙"}</span>
                <span>{theme==="dark"?"Modo claro":"Modo escuro"}</span>
              </div>
              <div style={{height:1,background:G.border,margin:"4px 0"}}/>
              <div onClick={()=>{onLogout();setMenuOpen(false);}} className="press"
                style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:10,cursor:"pointer",fontSize:14,fontWeight:500,color:G.red}}>
...
function ChatView({lancs,onAddLanc}){
  ...
  async function send(txt){
    ...
    }catch(e){push("ai","❌ "+((e?.message)||"Problema de conexão"));}  
    setBusy(false);
  }
  function confirmar(){
    if(!pending)return;
...
export default function App(){
  ...
  return(<>
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:G.bg}}>
      <Head view={view} onRec={()=>openModal("Receita")} onDep={()=>openModal("Despesa")} user={user} onLogout={handleLogout} theme={theme} onToggleTheme={toggleTheme}/>
      {dataLoading?(
        ...
        <main key={view} style={{position:"fixed",top:HH,left:0,right:0,bottom:NH,overflowY:"auto",overflowX:"hidden",padding:"16px 14px",WebkitOverflowScrolling:"touch",overscrollBehavior:"contain",animation:"fadeUp .2s ease both"}}>
          ...
          {view==="financas"&&<ErrorBoundary><FinancasView uid={user.uid} lancs={lancs}/></ErrorBoundary>}
        </main>
      )}
